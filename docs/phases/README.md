...
- **AR-9 — Consent boundary & suggestion provenance (decided 2026-06-12).** Explicit capture is a *consent boundary*, not a friction bug to engineer away. The capture aperture grows only through explicit user acts whose coverage then continues automatically (CC the agent once → the whole thread is followed; a user-owned Gmail filter → that sender is forwarded; a connected calendar → meeting metadata). Keeps never silently widens what it reads. Corollary — the **suggestion provenance test**: every proactive suggestion must be explainable in one digest sentence from (a) data the user explicitly captured, (b) time/absence signals derived from it, or (c) low-sensitivity connected metadata (e.g. calendar attendees/times). "Because we scanned your inbox" fails the test; features that require it are out of scope regardless of phase. Coverage must stay legible to the user (the digest states what Keeps is and isn't tracking) so missed-but-never-captured items cannot read as product failures.
...
| 5 ✅ | `phase-5-generated-insight-views.md` | **DONE.** Generated report records + signed 7-day expiring URLs, memo-style `/r/<token>` report pages with per-row actions, insight email commands (`what are my insights?` / waiting-on / stale / weekly / entity). Shared `mutateLoopState` funnel (web row action ≡ email command). Live-verified on keeps.email. | 3 |
| 6 ✅ | `phase-6-reliability-eval-trust.md` | **CODE DONE + DEPLOYED to keeps.email (2026-06-13).** Eval fixture suite + precision gate, Sentry (DSN-gated), model-call logs, delete/export/retention trust controls, dead-letter queue + replay, per-fn retries, connector alerting, `/admin/*` dashboards. Migrations 0015–0017 applied to prod; `is_admin` set. Deferred (Arav, non-blocking): Sentry DSN, 15 real eval cases, pointing the Postmark bounce webhook. | 3 (5 ideally) |
