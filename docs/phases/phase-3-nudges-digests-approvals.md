...
8. **Digest categorization.**
   ...
   Two AR-9 requirements (added 2026-06-12):
   - **Coverage line.** The digest opens with a one-line coverage statement, e.g. `Tracking 7 loops across 4 threads — Keeps sees only what you've shared.` Counts derive from the user's active loops and distinct threads. This makes the consent boundary legible so never-captured items cannot read as product failures.
   - **Capture prompt.** The digest closes with a scheduled capture moment: `What else is on your plate? Reply and I'll track it.` A free-text reply to the digest that is not a recognized command routes through the intent router's `capture` branch (the existing extraction path) — the daily digest thereby doubles as the zero-friction capture ritual, the cheapest counter to receive-time capture decay. Acceptance: replying to a digest with a plain-text brain dump creates loops for the user; replying `done 2` still resolves ordinal 2.
