...
- **Inngest functions weren't registered.** The Vercel↔Inngest integration's auto-sync did not register `process-email` against the live endpoint, so `email.received` events created no runs (silent — event accepted, no invocation). Root cause (confirmed 2026-06-12): the integration syncs by PUT-ing the **deployment-specific URL** (`keeps-<hash>-….vercel.app`), and the project's Deployment Protection was `all_except_custom_domains` with no automation-bypass secret — so every auto-sync died at Vercel's login wall before reaching our handler (zero `PUT /api/inngest` in logs until a manual sync). **Permanent fix applied:** protection scoped to `preview` deployments only (prod deployment URLs serve the same public app as the alias) + `INNGEST_SERVE_ORIGIN=https://keeps-ivory.vercel.app` in prod env so the app registers under the stable alias. Verified: the very next deploy received the integration's GET+PUT with 200 — auto-sync works, no manual step. (Alternative if protection must return to all-deployments: Vercel "Protection Bypass for Automation" secret pasted into the Inngest dashboard's Vercel settings; the team is on Pro so it's available.) Manual fallback: `npx inngest-cli@latest api --prod sync-app keeps --url https://keeps-ivory.vercel.app/api/inngest`.
