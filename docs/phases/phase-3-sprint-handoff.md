...
Wave D — app surface, parallel with C, 1 agent:
  D1 [sonnet]: approval pages (app/approvals/[id]/page.tsx + decide route + links.ts; token-only for requires_login=false, Clerk session redirect otherwise) + settings page for digest prefs/timezone (browser-detected tz via Intl). Keep the warm cream/terracotta design language from app/get-started-stepper.tsx.
